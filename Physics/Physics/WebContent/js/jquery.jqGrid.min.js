	...
	filterToolbar : function(p){
		...
		return this.each(function(){
			...
			var triggerToolbar = function() {
				var sdata={}, j=0, v, nm, sopt={},so,searchType= new Array(),st;//查询数据类型，查询属性 
				$.each($t.p.colModel,function(){
					...
					if(v || so==="nu" || so==="nn") {
						sdata[nm] = v;
						sopt[nm] = so;
						st = this.searchtype;//属性
						//console.log(st);//输出数据类型
						searchType.push(st);//记录属性类型
						j++;
					} else {
						try {
				...
				if(p.stringResult === true || $t.p.datatype === "local") {
					...
					$.each(sdata,function(i,n){
						...
						ruleGroup += "{\"field\":\"" + i + "\",";
						ruleGroup += "\"op\":\"" + sopt[i] + "\",";
						ruleGroup +="\"type\":\""+searchType[i]+"\",";//数据类型
						n+="";
						ruleGroup += "\"data\":\"" + n.replace(/\\/g,'\\\\').replace(/\"/g,'\\"') + "\"}";
						gi++;
					});
					ruleGroup += "]}";
					searchRule = ruleGroup;//存贮查询条件
			...
			clearToolbar = function(trigger){
				...
				if(p.stringResult === true || $t.p.datatype === "local") {
					...
					$.each(sdata,function(i,n){
						...
						ruleGroup += "{\"field\":\"" + i + "\",";
						ruleGroup += "\"op\":\"" + "eq" + "\",";
						ruleGroup +="\"type\":\""+searchType[i]+"\",";
						n+="";
						ruleGroup += "\"data\":\"" + n.replace(/\\/g,'\\\\').replace(/\"/g,'\\"') + "\"}";
						gi++;
					});
					ruleGroup += "]}";
					searchRule = ruleGroup;//备份查询条件
					//console.log(searchRule);
					$.extend($t.p.postData,{filters:ruleGroup});
					$.each(['searchField', 'searchString', 'searchOper'], function(i, n){
						if($t.p.postData.hasOwnProperty(n)) { delete $t.p.postData[n];}
		...
		*/
		...
		this.createTableForGroup = function(group, parentgroup) {
			...
			inputAddRule.bind('click',function() {
				...
				//console.log(cm.searchtype);//输出查询类型
				//console.log(cm.name);//输出查询列名
				group.rules.push({
					field: cm.name,
					op: opr[0],
					type: cm.searchtype,//查询类型
					data: ""
				}); // adding a new rule
			...
			ruleFieldSelect.bind('change',function() {
				...
				for (i=0;i<that.p.columns.length;i++) {
					if(that.p.columns[i].name ===  rule.field) {
						cm = that.p.columns[i];
						rule.type=cm.searchtype;//更新rule数据类型
						break;
					}
				}
				if(!cm) {return;}
		...
		this.getStringForRule = function(rule) {
			...
			numtypes = ['int', 'integer', 'float', 'number', 'currency','datetime','yearmonth','year','percent']; // jqGrid 添加时间、年月、年、名次与比例
	...
	searchGrid : function (p) {
		...
		return this.each(function() {
			...
			if ( $("#"+$.jgrid.jqID(IDs.themodal))[0] !== undefined ) {
				...
				bQ = "", tmpl="", colnm, found = false, bt, cmi=-1,sType;//查询数据类型
				if(p.showQuery) {
				...
				if(!p.columns.length) {
					$.each(columns, function(i,n){
						...
						if(!found) {
							...
							if ((ignoreHiding && searchable) || (searchable && !hidden)) {
								...
								//console.log(colnm);//输出数据名称
								sType = n.searchtype;//查询数据类型
								//console.log(sType);//输出数据类型
								cmi =i;
							}
						}
					});
					...
					sType = columns[0].searchtype;//查询数据类型
					//console.log(sType);//输出数据类型
				}
				// old behaviour
				if( (!defaultFilters && colnm) || p.multipleSearch === false  ) {
					...
					defaultFilters = {groupOp: "AND", rules: [{field: colnm, op: cmop,type:sType, data: ""}]};//添加查询类型
				}
				found = false;
				...
				$("#"+fid+"_search").bind('click', function(){//点击查询函数
					var fl = $("#"+fid),
					sdata={}, res,
				...
				$("#"+fid+"_reset").bind('click', function(){
					//alert('inner reset!');查询过滤条件重置
					isAllData = true;//清除过滤条件
					var sdata={},
					fl = $("#"+fid);
					$t.p.search = false;

 ...
 * @param type
 */
function notPubCollectItem(id,type){
	var eleId='';
	if(type=='0'){
		eleId='jqGrid_collect_tb';
	}
	...
	$.ajax({
		url:unPrepubUrl,
		type:'POST',
		data:'entityId='+tableId+'&versionId='+versionId+'&pkValue='+primaryKey,
		success : function(data) {
			if (data) {
				alert_dialog("不公示成功");
				$("#"+eleId).trigger("reloadGrid");
			}
		}
	});
}
 ...
 * @param type
 */
function pubCollectItem(id,type){
	var eleId='';
	if(type=='0'){
		eleId='jqGrid_collect_tb';
	}
 ...
 * @param type
 */
function checkPubCollectItem(id,type,tableName){
	var eleId='';
	var contentKey;
	if(type=='0'){
		eleId='jqGrid_collect_tb';
	...
	var rowData = $("#"+eleId).jqGrid("getRowData",id);
	var unitId = rowData['UNIT_ID'];
	var discId = rowData['DISC_ID'];
	$.ajax({
		url:checkInfoUrl,
		type:'POST',
		data:'entityId='+tableId+'&unitId='+unitId+'&discId='+discId,
		dataType:'json',
		success : function(data) {
			if (data) {
				initINFOialog(data,tableName);
			}
		}
	});
}
 ...
 * @param tableName
 */
function checkObjectCollectItem(id,tableName,eleId){
	//console.log(eleId);
	var rowData = $("#"+eleId).jqGrid("getRowData",id);
	var unitId = rowData['UNIT_ID'];
	var discId = rowData['DISC_ID'];

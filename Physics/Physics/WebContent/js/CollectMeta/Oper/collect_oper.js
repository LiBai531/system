...
function addCollectItemPart(type){
	...
	if(jsCheckFunFactory2(tableName,records)){
...
function setRowControllers(id){
	...
	//日期控件+在读setYearMonth2Col
	setYearMonth2Col(controllerDic.YEARMONTH2,id);
	//查询选择
	...
	//下拉字典中的其中一个字典是下拉
	setSelectLinkSelectCol(controllerDic.SELECTLINKSELECT,id);
	
}
function removeWarnInfo(id){
	$.each(controllerDic.MULSELECT,function(i,item){
		var v = $("#"+id+"_"+item).val();
		if($.trim(v)=="点击进行选择"){
			$("#"+id+"_"+item).val("");
		}
	});
	$.each(controllerDic.SEARCHSELECT,function(i,item){
		var v = $("#"+id+"_"+item).val();
		if($.trim(v)=="点击进行选择"){
			$("#"+id+"_"+item).val("");
		}
	...
	jQuery("#"+eleId).jqGrid('saveRow',id,
	  {
		...
		if(data){
			...
			if(data.responseText!='SAVE_ERROR'&&data.responseText!='UPDATE_ERROR'){
				...
				if(interfereType=="SELF"||interfereType=="BOTH"){
					jQuery("#"+eleId).jqGrid('setGridParam').trigger("reloadGrid");
				}
				return true;
			}else
				{
 ...
 * @param id
 */
var isCancelItem= true;//类似于信号量用于同步
 ...
 * 给单独删除添加判断
 */
function delCollectItem2(id,type){
	//alert('删除id : '+id);
	//alert('新添加记录数： '+newAddSeqNos.length);
	if(newAddSeqNos.length>0){
		$("#dialog_confirm").empty().append("<p>有数据还没有保存,</p><br/>" +
 ...
 */
function excel_output(unitId,discId,roundId,tableId,sortOrder,sortName,searchRule)
{
	...
	if(searchRule==''){
		...
		document.getElementById("excel_baseSearch_output").href=url+"?filters="+encodeURI(JSON.stringify(searchRule));
	}else{
		document.getElementById("excel_baseSearch_output").href=url+"?filters="+encodeURI(JSON.stringify(searchRule));
	}	
}
...
/*	newAddSeqNos.splice($.inArray(id,newAddSeqNos),1);
	jQuery(".jqgrid-new-row").each(function(){
		var tmpId=jQuery(this).attr("id");
		if(isHasItem(tmpId,newAddSeqNos)=='-1'){
			jQuery('#'+eleId).jqGrid('delRowData',tmpId);
		}	
	});
	
	for(var i=0;i<newAddSeqNos.length;i++){
		jQuery("#"+eleId).jqGrid('setRowData',newAddSeqNos[i],{SEQ_NO :records-newAddSeqNos.length+i+1});
	}
	iscancelNewItem = true;*/
	}
}

function isHasItem(id,newAddSeqNos){
	for(var i=0;i<newAddSeqNos.length;i++){
		if(newAddSeqNos[i]==id){
			return i;
		}
	}
	return -1;
